...
<div
	class="flex flex-col gap-4 rounded-xl border border-stone-200 bg-light-one_d p-4 shadow-sm transition-all hover:border-stone-300"
>
	<div class="flex items-start justify-between gap-3">
		<div class="flex items-center gap-3">
			...
			{/if}
			<div>
				<h3 class="line-clamp-1 font-semibold text-stone-800">
					{enrollment.course?.title || 'Curso desconocido'}
				</h3>
				<p class="text-sm text-stone-500">
					{enrollment.user?.full_name || enrollment.user?.username || 'Usuario desconocido'}
				</p>
			</div>
		</div>
	...
	<div class="grid grid-cols-2 gap-4 border-t border-stone-100 pt-3">
		<div>
			<p class="text-[10px] font-medium tracking-wider text-stone-400 uppercase">Inscripción</p>
			<p class="text-sm font-medium text-stone-700">{formatDate(enrollment.enrolled_at)}</p>
		</div>
		<div>
			<p class="text-[10px] font-medium tracking-wider text-stone-400 uppercase">Vence</p>
