		...
		<BlurOverlay class="flex items-center justify-center p-4">
			...
			>
				...
				<div class="custom-scrollbar max-h-[70vh] space-y-8 overflow-y-auto p-6 sm:p-8">
					<div class="grid gap-6 md:grid-cols-2">
						...
						<div class="md:col-span-2">
							...
							<div class="group relative">
								<div
									class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-stone-400 transition-colors group-focus-within:text-stone-900"
								>
								...
								<Input
									...
									bind:value={title}
									class="pl-10 transition-all focus:ring-2 focus:ring-stone-900/5"
								/>
							</div>
						</div>
						...
						<!-- Description -->
						<div class="md:col-span-2">
							<label class="mb-2 block text-sm font-bold text-stone-700" for="description"
								>Descripción * (mín. 20 caracteres)</label
							>
							<TextArea
								id="description"
							...
							<div class="mt-1 flex justify-end">
								<span
									class="text-[10px] font-medium {description.length < 20
										? 'text-rose-500'
										: 'text-emerald-500'}"
								>
						...
						<div class="space-y-1.5">
							<label class="block text-sm font-bold text-stone-700" for="category"
								>Categoría *</label
							>
							<Select
								bind:value={category}
						...
						<!-- Subcategory -->
						<div class="space-y-1.5">
							<label class="block text-sm font-bold text-stone-700" for="subcategory"
						...
						<!-- Difficulty -->
						<div class="space-y-1.5">
							<label class="block text-sm font-bold text-stone-700" for="difficulty"
								>Dificultad *</label
							>
							<Select
						...
						<div class="space-y-1.5">
							...
							<div class="group relative">
								<div
									class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-stone-400 group-focus-within:text-stone-900"
								>
						...
						<!-- WhatsApp Group URL -->
						<div class="md:col-span-2">
							<label class="mb-2 block text-sm font-bold text-stone-700" for="whatsapp"
								>Link Grupo de WhatsApp *</label
							>
							<div class="group relative">
								<div
									class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-stone-400 group-focus-within:text-green-600"
								>
					...
					{#if !course}
						<div
							class="flex items-start gap-3 rounded-2xl border border-rose-100 bg-rose-50/50 p-4"
							transition:fade
						>
							...
							<p class="text-xs leading-relaxed font-medium text-stone-600">
								...
								<span class="font-bold tracking-wider text-rose-600 uppercase"
									>Borrador (DRAFT)</span
					...
					{:else}
						<div
							class="flex items-start gap-3 rounded-2xl border border-stone-100 bg-stone-50/50 p-4"
							transition:fade
						>
							<RefreshIcon class="mt-0.5 size-5 shrink-0 text-stone-400" />
							<p class="text-xs leading-relaxed font-medium text-stone-600">
								Estás editando el curso <span class="font-bold text-stone-900"
									>"{course.title}"</span
								>. Los cambios se aplicarán inmediatamente (si el curso está publicado, los alumnos
								verán los cambios).
							</p>
						</div>
					{/if}
				...
				<!-- Footer Actions -->
				<div
					class="flex flex-col gap-3 border-t border-stone-100 bg-stone-50/50 px-8 py-6 sm:flex-row"
				>
					<Button
						variant="outline"
						class="order-2 flex-1 border-stone-200 py-4 font-bold transition-all hover:border-stone-300 hover:bg-white sm:order-1"
						onclick={handleClose}
					>
						Cancelar
					</Button>
